...
| X25 | EXT-SYS-006 外部排序、取消、资源 | 部分实现 | 外部排序/聚合、NDJSON、drain 背压、会话取消、结果预算、工作台字段；执行器 `RowStream`、`Scan/Filter/Project/LimitRowStream` 与 HTTP `resourceUsage` 回传（15 项检查通过） | 排序/聚合逐行流、跨进程逐行输出、提前停止、完整压力验收 |
