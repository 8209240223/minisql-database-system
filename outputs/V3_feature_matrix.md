...
| X24 | EXT-SYS-005 权限与审计 | 部分实现 | 页式访问目录、角色继承、对象授权、加盐密码、身份绑定、原子 HTTP 端点、CLI 身份、权限/审计面板；C++ session 与直连二进制已校验身份和对象权限，权限版本支持热重载 | 访问目录尚未成为现有 PersistentCatalog 的系统表；复杂 SQL 对象身份仍依赖轻量扫描 |
...
| X27 | EXT-QA-001 Fuzz 与长期回归 | 部分实现 | SELECT 差分 500 项、DDL/DML 状态机、3 种固定种子 96 步回归、2 种固定种子 512 步长跑、逐文件语料复现、失败 artifact 重放入口、失败分类、DATE/BOOL 回归、五阶段跨进程崩溃恢复组合、4 种子 × 512 步 × 4096 条压力数据的溢写/资源回归、CI 入口 | 无限输入、多小时长期运行资源趋势 |

## 本轮新增闭环

- X24：`access-catalog.mjs` 的原子用户/角色/授权操作映射到 HTTP 资源端点；CLI 通过 HTTP bridge 携带身份；C++ session 和直连二进制入口已读取 `access.catalog.pages`，执行前校验身份/对象权限并按权限版本热重载。
- X27：状态机差分脚本优先选择 `build/windows/Release/minisql_database.exe`，小规模、3 种 96 步回归和 2 种 512 步长跑均通过；新增 DATE/BOOL 字面量规划回归、五阶段跨进程崩溃恢复组合和 4 种子 × 512 步 × 4096 条压力数据的溢写/资源回归，固定种子逐文件复现契约、失败 artifact 重放入口和 Windows 引擎回归均已接入 CI/本地验收链。
