...
| X26 | EXT-SYS-007 备份恢复与迁移 | 部分实现 | v2 全量备份、v1 迁移、v3 增量链、v4 在线一致性快照、活动事务未提交时快照隔离、非零 WAL 截止位置重做、恢复回滚目录与失败自动还原、页文件校验和原子恢复；备份 smoke 50 项、在线快照 22 项检查通过 | 流式页面恢复、在线恢复一致性仍需补齐 |
