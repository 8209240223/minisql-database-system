...
| X09 | EXT-SQL-009 子查询 | 部分实现 | IN、NOT IN、EXISTS、标量、限定名相关子查询、派生表、NULL、UPDATE/DELETE；derived-smoke 12 项通过 | 未限定相关作用域、Apply/SemiJoin 和全量组合 |
...
| X24 | EXT-SYS-005 权限与审计 | 部分实现 | 页式访问目录、角色继承、对象授权、加盐密码、身份绑定、原子 HTTP 端点、CLI 身份、权限/审计面板 | 访问目录进入 C++ 内部 Catalog；直接二进制入口仍不鉴权 |
...
| X27 | EXT-QA-001 Fuzz 与长期回归 | 部分实现 | SELECT 差分 500 项、DDL/DML 状态机、3 种固定种子 96 步回归、2 种固定种子 512 步长跑、逐文件语料复现、失败 artifact 重放入口、失败分类、DATE/BOOL 回归、CI 入口 | 无限/更高压力输入、崩溃恢复组合、长期运行资源趋势 |

## 本轮新增闭环

- X24：`access-catalog.mjs` 的原子用户/角色/授权操作映射到 HTTP 资源端点；CLI 通过 HTTP bridge 携带身份，不再直接绕过权限调用 C++ 二进制。
- X27：状态机差分脚本优先选择 `build/windows/Release/minisql_database.exe`，小规模、3 种 96 步回归和 2 种 512 步长跑均通过；新增 DATE/BOOL 字面量规划回归，固定种子逐文件复现契约、失败 artifact 重放入口和 Windows 引擎回归均已接入 CI。
- C2：工作台已加入用户/密码身份、权限/审计/会话、锁等待和取消、资源预算、备份列表与恢复操作入口；真实浏览器 DOM 回归已覆盖连接错误/恢复、身份请求头、权限会话、设置、侧栏拖动和 390px 移动端无溢出。
