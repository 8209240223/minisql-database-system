...
| X25 | EXT-SYS-006 外部排序、取消、资源 | 部分实现 | 外部排序/聚合、NDJSON、drain 背压、会话取消、结果预算、工作台字段；执行器 `RowStream`、`Scan/Filter/Project/LimitRowStream`、`MaterializedRowStream` 与 HTTP `resourceUsage` 回传（15 项检查通过），Limit 已提前停止 | 真实排序/聚合逐行生产、跨进程逐行输出、完整压力验收 |
...
| X25 | B/C | `src/execution/external_sort.cpp`、`src/execution/database.cpp`、`src/execution/executor.hpp`、`scripts/database-bridge.mjs`、`src/server/database_main.cpp` | `node tests/external-sort-smoke.mjs`；`node tests/external-aggregate-smoke.mjs`；`node tests/cancel-smoke.mjs`；`node tests/result-budget-smoke.mjs`；`node tests/x25-stream-http.mjs`；`node tests/x25-row-stream-contract.mjs` | 外部 run、NDJSON、背压、取消、结果预算和工作台字段已有证据；`RowStream` 链、Sort/Aggregate/Distinct 物化流和 HTTP 资源回传已接入，真实排序/聚合逐行生产与跨进程逐行输出仍部分实现 |
