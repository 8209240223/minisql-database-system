...
| X25 | EXT-SYS-006 外部排序、取消、资源 | 部分实现 | 外部排序/聚合、NDJSON、drain 背压、会话取消、结果预算、工作台字段；执行器 `RowStream` 接口与 `ScanRowStream` 资源计量 | C++ 执行器级迭代、提前停止、完整压力验收 |
...
| X25 | B/C | `src/execution/external_sort.cpp`、`src/execution/database.cpp`、`src/execution/executor.hpp`、`scripts/database-bridge.mjs`、`src/server/database_main.cpp` | `node tests/external-sort-smoke.mjs`；`node tests/external-aggregate-smoke.mjs`；`node tests/cancel-smoke.mjs`；`node tests/result-budget-smoke.mjs`；`node tests/x25-stream-http.mjs` | 外部 run、NDJSON、背压、取消、结果预算和工作台字段已有证据；`RowStream` 与 `ScanRowStream` 已接入接口层，执行器级迭代/提前停止仍部分实现 |
