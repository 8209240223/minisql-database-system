...
| EXT-SQL-009 | 子查询、IN、EXISTS、标量子查询 | 多层作用域、Apply | 部分实现：非相关 IN/NOT IN、EXISTS、标量子查询、限定名相关子查询和派生表已接通，覆盖 NULL、空结果、投影、UPDATE/DELETE、ANALYZE 和错误拒绝；未限定相关作用域、Apply/SemiJoin 和全量组合待完成 | X09 |
...
| EXT-CMP-002 | 可扩展 AST、LR/生成器方案 | AST 接口、序列化 | 部分实现：AST/Plan/Catalog schema versioning 与迁移入口已接入，并有解析器回归；完整 AST 扩展机制、LR/生成器方案和稳定迁移契约待完成 | X13 |
...
| 编号 | 功能 | 依赖 | 当前状态 | 验收编号 |
| --- | --- | --- | --- | --- |
| EXT-SYS-001 | B+ 树、复合/唯一索引、IndexScan | Page、RowId、Catalog | 部分实现：页级 B+ 树已成为默认索引主路径，支持元页/节点页、根到叶遍历、根分裂、范围查询、删除借位/合并、重启恢复、结构 inspect 和强制重建；29+41+44 项索引检查通过，新增 1000 行（height=2, pages=33）和 10000 行（height=3, pages=323）规模回归。更大规模性能曲线和完整 X20 验收待完成 | X20 |
| EXT-SYS-002 | BEGIN、COMMIT、ROLLBACK、原子性 | WAL、锁或版本 | 部分实现：显式事务、事务化 DDL、常驻 HTTP 会话及工作台事务控制；进程/HTTP 测试和真实浏览器提交、回滚、失败、过期、响应丢失检查通过；新增 `SAVEPOINT` / `RELEASE` / `ROLLBACK TO`，DML、DDL 和索引恢复专项 7 项通过；多逻辑会话采用数据库级排他两阶段锁，锁等待、超时和关闭回滚已接通，行级隔离/MVCC、事务溢写和 X21 全量组合验收未完成 | X21 |
...
| EXT-SYS-006 | 外部排序、大结果集、取消和资源预算 | Sort、Buffer | 部分实现：ORDER BY 超过内存行预算时写入带 sessionId、查询/排序序号和 FNV-1a 校验的 JSONL run 并多路归并，临时文件结束或异常清理，专项检查 13 项通过；分组输入超过预算时按分组键外部排序并合并状态，外部聚合专项检查 8 项通过；新增只读 NDJSON 流式接口和 TCP drain 背压路径，12 项 X25 流式检查通过；新增执行器 `RowStream`、`Scan/Filter/Project/LimitRowStream`、`MaterializedRowStream`、`openRowStream`、Limit 提前停止和 HTTP `resourceUsage` 回传，资源契约 15 项通过。真实排序/聚合逐行生产、跨进程逐行输出和完整 X25 验收待完成，详见 `docs/streaming-progress.md` | X25 |
| EXT-SYS-007 | 备份恢复、格式版本和迁移 | 持久化、WAL | 部分实现：v2 全量备份、v1 迁移、v3 离线增量链和链路深度已接通；新增 v4 在线一致性快照、活动事务隔离快照、WAL/`.ckpt` sidecar 复制、非零 WAL 截止位置恢复、恢复回滚目录、失败自动还原和页校验，备份 56 项、在线快照 22 项检查通过；在线恢复一致性和流式页面恢复仍需完整 X26 验收 | X26 |
