...
| EXT-SYS-007 | 备份恢复、格式版本和迁移 | 持久化、WAL | 部分实现：v2 全量备份、v1 迁移、v3 离线增量链已接通；新增 v4 在线一致性快照、WAL/`.ckpt` sidecar 复制、非零 WAL 截止位置恢复、恢复回滚目录和页校验，在线快照 19 项检查通过；在线恢复一致性、流式页面恢复、自动迁移失败回滚目录和完整 X26 验收待完成 | X26 |
