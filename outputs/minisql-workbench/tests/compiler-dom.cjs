...
(async () => {
  ...
  try {
    ...
    assert.match(await page.locator('.cm-content').innerText(), /SELECT/);
    ...
    await page.getByRole('button', { name: /^Token 流/ }).click();
    await page.getByRole('table', { name: 'Token 流' }).waitFor();
    assert.ok(await page.getByRole('table', { name: 'Token 流' }).locator('tbody tr').count() > 20);
    const firstKeyword = page.locator('.token-row').first();
    await firstKeyword.click();
    assert.equal(await page.evaluate(() => window.getSelection()?.toString() ?? ''), 'CREATE');
    ...
    await page.locator('.plan-node-details summary').first().click();
    assert.match(await page.locator('.plan-node-details').first().innerText(), /输入节点/);
    assert.match(await page.locator('.plan-node-details').first().innerText(), /输出列/);
