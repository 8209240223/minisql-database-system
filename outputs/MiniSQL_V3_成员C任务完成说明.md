...
**成员 C 待完成**
- 将未限定相关作用域和更广 SQL 的完整语义绑定接入统一 Catalog/AST 结果；当前解析失败兜底只负责保守识别基础对象，不替代完整语义分析。
...
- C 任务仍不能标记为“全部完成”：X24 的未限定相关作用域和更广 SQL 语义绑定仍有边界，X27 的无限输入和多小时长期资源趋势仍需长期环境证据；同时 X01-X27 仍有其他部分实现项。
