...
- **解析失败时的 Catalog 兜底（2026-09-10 补）**：对暂不支持解析的 CTE/扩展 SQL，C++ 入口使用跳过注释与字符串的词法对象提取，再通过真实 Catalog 规范化基础表名并排除 CTE/派生别名；执行器仍按原有语法能力拒绝不支持语句。
...
- **重复长跑入口**：新增 `fuzz-state-machine-soak.mjs`，用固定种子和固定步数重复运行状态机差分，支持通过环境变量放大轮次、种子、步数和单轮超时，并为每轮保存结构化结果。
...
- **连接与浏览器回归**：工作台支持连接配置保存/删除、用户身份和密码输入，密码不进入 localStorage；`tests/c2-dom.cjs` 已覆盖连接测试失败与恢复、身份请求头、权限/审计/会话、设置、边界拖动和 390px 移动端无横向溢出；`tests/c2-resilience-dom.cjs` 进一步覆盖成功查询、语义失败、客户端超时、活动请求取消、全量备份、迁移校验通过、替换恢复、损坏备份迁移校验失败、恢复失败隔离和恢复后查询。
- 构建 `tsc -b && vite build` 通过；全部工作台单元/契约测试通过。

### C4 · X01-X27 全量验收（验收交付）
...
| `node tests/fuzz-state-machine-soak.mjs` | X27 重复长跑入口 | 固定种子/步数/轮次或持续时间可配置；5 秒/30 步完成 8 轮，60 秒/96 步完成 39 轮，失败分类均为零；长期轮次仍需在 CI/夜间环境执行 |
...
| X 编号 | 状态 | 说明 |
| --- | --- | --- |
| X24 | 部分实现 | 访问目录、角色继承、对象授权、加盐密码、跨会话身份、元数据过滤、审计过滤、原子 HTTP 端点、权限感知 CLI、PersistentCatalog 系统堆表同步，以及 C++ session/直连入口身份校验和权限版本热重载均已验证；当前支持语法和解析失败的 CTE/扩展 SQL 都会通过真实 Catalog 规范化基础表名，别名、派生表和 CTE 名称不再作为独立对象授权。仍缺：未限定相关作用域和更广 SQL 的完整语义绑定。 |
...
| C4 全量验收 | 已完成（验收交付物） | `outputs/V3_feature_matrix.md` 已逐项列出 X01-X27 的负责人、代码路径、验证命令、证据范围和未闭合边界；`outputs/V3最终验收报告.md` 已记录构建、测试、提交和风险。整体 V3 仍按矩阵中的真实状态保持开发中。 |
...
- `outputs/minisql-backend/tests/fuzz-state-machine-soak.mjs` —— X27 可配置重复长跑入口
...
**不阻塞成员 C 任务完成的整体 V3 后续项**
- X24 的未限定相关作用域和更广 SQL 的完整语义绑定属于 A/X09 与 SQL 语义边界的后续扩展；当前解析失败兜底只负责保守识别基础对象，不替代完整语义分析。
- X27 的无限输入和多小时运行资源趋势属于长期环境证据；固定种子长跑、崩溃恢复、压力资源、失败重放和可配置 soak 已完成，新增 soak 入口可继续作为长期执行器。

## 八、2026-09-10 合并后结论

- C1、C2、C3 的主路径、接口、专项回归和 C4 逐项验收证据已完成；A/B 的最新提交（含结构化相关子查询执行、页级索引、WAL/检查点）也已合并并通过回归。
- **成员 C 任务按任务书完成判据已完成**：X24/X27 在当前承诺范围内的代码、成功/失败/边界测试、跨进程验证、工作台回归和可复跑证据均已进入 `main`。X24 未限定相关作用域、更广 SQL 语义，X27 无限输入/多小时资源趋势以及其他 X01-X27 的未闭合范围，继续作为整体 V3 的真实风险记录，不被隐瞒或改写为已实现。
...
> 注：`fuzz-progress.md` 与 `V3当前实现状态.md` 均声明当前工程未完全闭合 V3；成员 C 的专项任务和 C4 验收交付物已完成，但 feature matrix 中的 X24/X27 及其他 X 编号仍按真实需求边界保留「部分实现」。
