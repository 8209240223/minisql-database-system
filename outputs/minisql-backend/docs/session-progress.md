...
默认空闲超时 300000 毫秒，MINISQL_SESSION_IDLE_MS 可配置为 100 至 3600000 毫秒。超时排队关闭并回滚，不中断执行中的请求；活动请求、锁等待或持有事务锁时只顺延空闲计时，操作结束后重新计算空闲窗口；旧超时回调通过代次标记失效。单次引擎请求默认超时 30000 毫秒，可由 MINISQL_ENGINE_REQUEST_TIMEOUT_MS 配置为 1000 至 3600000 毫秒。执行中的 HTTP 连接异常断开时停止会话并隔离桥接服务，提交结果按未知处理，不假装已确定回滚。
