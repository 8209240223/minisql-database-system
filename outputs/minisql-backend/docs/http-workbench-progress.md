...
| 前端 npm.cmd run test:browser | Edge 无头浏览器编译工作台 + 连接/权限/设置/拖拽/移动端 + 真实取消/备份恢复回归通过 |

浏览器测试实际操作连接管理、编辑器、编译、AST、Diagnostics、计划切换、权限与审计、会话状态、设置入口、侧栏边界拖动和移动端视口；`c2-resilience-dom.cjs` 另使用隔离数据库实际执行成功查询、语义失败、客户端超时、活动请求取消、全量备份、替换恢复、损坏备份失败隔离和恢复后查询，并检查无页面脚本异常。测试只检查 DOM 和接口，不生成或读取图片。
...
5. 前端已补充连接身份、权限/审计/会话、锁等待取消、资源预算、客户端请求超时和备份入口；浏览器已覆盖活动查询取消、客户端超时、真实备份替换、迁移校验成功、损坏备份迁移校验失败、恢复失败隔离和恢复后查询。迁移接口已返回明确的 operation/phase/errorCode，未把失败伪装成普通恢复失败。
