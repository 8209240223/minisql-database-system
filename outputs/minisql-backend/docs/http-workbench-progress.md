...
浏览器测试实际操作连接管理、编辑器、编译、AST、Diagnostics、计划切换、权限与审计、会话状态、设置入口、侧栏边界拖动和移动端视口；`c2-resilience-dom.cjs` 另使用隔离数据库实际执行成功查询、语义失败、活动请求取消、全量备份、替换恢复和恢复后查询，并检查无页面脚本异常。测试只检查 DOM 和接口，不生成或读取图片。
...
5. 前端已补充连接身份、权限/审计/会话、锁等待取消、资源预算和备份入口；浏览器已覆盖活动查询取消、真实备份替换恢复和恢复后查询。引擎超时仍由后端超时/资源回归覆盖，前端没有独立的客户端超时控制。
