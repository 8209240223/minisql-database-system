...
equal(tableReferences('WITH visible(id) AS (SELECT id FROM secret_records) SELECT id FROM visible;', 'SELECT'), ['secret_records'], 'named-column CTE aliases are not treated as persisted tables');
